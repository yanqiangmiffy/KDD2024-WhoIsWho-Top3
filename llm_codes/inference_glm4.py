...
class IND4EVAL(Dataset):
    ...
    def __getitem__(self, index):
        ...
        input_ids = [151331, 151333] + self.user_begin_tokens + human_tokens + self.assistant_begin_tokens
...
with torch.no_grad():
    for index,batch in tqdm(enumerate(val_data)):
        ...
        if not hasattr(model, 'module'):
            response = model.generate(**batch_input, max_length=batch_input['input_ids'].shape[-1] + 16 + 5, return_dict_in_generate=True, output_scores=True)
        else:
            response = model.module.generate(**batch_input, max_length=batch_input['input_ids'].shape[-1] + 16 + 5, return_dict_in_generate=True, output_scores=True)
